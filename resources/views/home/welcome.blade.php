...
@endsection
@section('content')
<div class="mb-2 home-slider slide-animate owl-carousel owl-theme show-nav-hover nav-big text-uppercase" data-owl-options="{
    'loop': true
}">
    @foreach ($adverts as $offer)
        <div class="home-slide home-slide1 banner">
            <img class="slide-bg" src="{{asset('storage/images/'. $offer->image)}}" width="1903" height="499" alt="slider image">
            <div class="container d-flex align-items-center">
                <div class="banner-layer appear-animate" data-animation-name="fadeInUpShorter">
                    ...
                    <a href="{{ route('offers',str_replace(' ', '-', $offer->title) ) }}" class="btn btn-dark btn-lg">Learn More!</a>
                </div>
                <!-- End .banner-layer -->
            </div>
        </div>
...
<div class="container">
    <div class="mb-2 info-boxes-slider owl-carousel owl-theme" data-owl-options="{
        'dots': false,
        'loop': false,
        'responsive': {
            '576': {
...
<section class="new-products-section">
    <div class="container">
        ...
        <div class="mb-2 products-slider custom-products owl-carousel owl-theme nav-outer show-nav-hover nav-image-center" data-owl-options="{
            'dots': false,
            'nav': true,
            'responsive': {
                '992': {
        ...
        <div class="product-default appear-animate" data-animation-name="fadeInRightShorter">
                <figure>
                    <a href="{{ route('productsingle',str_replace(' ', '-', $arrive->name) ) }}">
                        <img src="{{asset('storage/images/'. $arrive->featured_image)}}" width="220" height="220" alt="product">
                        <img src="{{asset('storage/images/'. $arrive->featured_image)}}" width="220" height="220" alt="product">
                    </a>
                    <div class="label-group">
                        <div class="product-label label-hot">HOT</div>
                ...
                <div class="product-details">
                    <h3 class="{{ route('productsingle',str_replace(' ', '-', $arrive->name) ) }}">
                        <a href="product.html">{{$arrive->name}}</a>
                    </h3>
                    
                    <!-- End .product-container -->
                    <div class="price-box">
                        <del class="old-price">{{$settings->currency}}{{number_format($arrive->previous_price)}}</del>
                        <span class="product-price">{{$settings->currency}}{{number_format($arrive->current_price)}}</span>
                    </div>
                    <!-- End .price-box -->
                    <div class="product-action">
                        @auth
                          <a href="javascript:void(0)" onclick="addtowish(this.id)" id="{{$arrive->id}}"  class="btn-icon-wish" title="wishlist"><i
                                class="icon-heart"></i></a>  
                        @endauth
                        @if ($arrive->instock < 1)
                            <a class="p-3 text-white btn-icon btn-danger"><span>SOLD OUT</span></a> 
                        @else
                           <a href="{{ route('productsingle',str_replace(' ', '-', $arrive->name) ) }}" class="p-3 text-white btn-icon btn-dark"><span>BUY NOW</span></a> 
                        @endif
        ...
        <div class="banner banner-big-sale appear-animate" data-animation-delay="200" data-animation-name="fadeInUpShorter" style="background: #2A95CB center/cover url('app/images/demoes/demo4/banners/banner-4.jpg');">
            <div class="mx-0 banner-content row align-items-center">
                ...
                <div class="text-center col-md-3 col-sm-4 text-sm-right">
                    <a class="btn btn-light btn-white btn-lg" href="{{route('products', ['cat'=>'all', 'brand' => 'all', 'st' => 'desc', 'pg' => 10, 'search'=> 'query'])}}">View Products</a>
                </div>
            </div>
        </div>
    </div>
